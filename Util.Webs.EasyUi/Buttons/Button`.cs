namespace Util.Webs.EasyUi.Buttons {
    ...
    public abstract class Button<T> : ButtonBase<T>, IButton<T> where T : IButton<T> {
        ...
        /// <param name="gridId">表格Id</param>
        public T AddByGrid( object row = null, string gridId = "" ) {
            return Click( string.Format( "$.easyui.grid.add({0})", GetAddParams( row, gridId ) ) );
        }
        ...
        /// <param name="gridId">表格Id</param>
        public T AddRootByTreeGrid( object row = null, string gridId = "" ) {
            return Click( string.Format( "$.easyui.treegrid.addRoot({0})", GetAddParams( row, gridId ) ) );
        }
